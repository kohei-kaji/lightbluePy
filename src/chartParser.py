...
def parse(beam: int, sentence: str) -> Chart:
    ...
    if sentence == "":
        ...
        chart, _, _, _ = reduce(
            lambda acc, c: chartAccumulator(beam, lexicon, acc, c),
            purifyText(sentence),
            ({}, [0], 0, ""),
        )
        return chart
...
def chartAccumulator(
    beam: int, lexicon: list[Node], partialChart: PartialChart, c: str
) -> PartialChart:
    ...
    if c == "、":
        ...
        newstack = c + stack
        newchart, _, _, _ = reduce(
            lambda acc, c: boxAccumulator(beam, lexicon, acc, c),
            newstack,
            (chart.copy(), "", i, i + 1),
        )
        newseps = (
...
def punctFilter(
    sep: int,
    i: int,
    charList: list[tuple[tuple[int, int], list[Node]]],
    e: tuple[tuple[int, int], list[Node]],
) -> list[tuple[tuple[int, int], list[Node]]]:
    from_, to = e[0]
    nodes = e[1]
    if to == i:
        return (
            ((from_, to + 1), list(filter(lambda n: CCG.isBunsetsu(n.cat), nodes))),
            e,
        ) + charList
    else:
        return e + charList
    ...
    return lexicalitem(
        ...
        cat.BS(
            cat.SL(
                ...
                cat.BS(cat.T(True, 1, modifiableS), cat.NP([feature.F([FV.Ga, FV.O])])),
            ),
            feature.NP([FV.F([FV.Nc])]),
        ),
    )
...
def boxAccumulator(
    ...
    # ここのソートを実装する
    newchart[(i, j)] = sorted(list1, key=lambda n: n.score, reverse=True)[:beam]
...
def checkUnaryRules(prevlist: list[Node]) -> list[Node]:
    return reduce(
        lambda acc, node: CCG.unaryRules(node, acc), prevlist, prevlist.copy()
    )


def checkBinaryRules(i: int, j: int, chart: Chart, prevlist: list[Node]) -> list[Node]:
    return reduce(
        lambda acck, k: reduce(
            lambda accl, lnode: reduce(
                lambda accr, rnode: CCG.binaryRules(lnode, rnode, accr),
...
def checkCoordinationRule(
    i: int, j: int, chart: Chart, prevlist: list[Node]
) -> list[Node]:
    return reduce(
        lambda acck, k: reduce(
            lambda accc, cnode: reduce(
                lambda accl, lnode: reduce(
                    lambda accr, rnode: CCG.coordinationRule(lnode, cnode, rnode, accr),
            ...
            filter(lambda n: n.cat == cat.CONJ, lookupChart(k, k + 1, chart)),
            acck.copy(),
        ),
        range(i + 1, j - 1),
        prevlist.copy(),
...
def checkParenthesisRule(
    i: int, j: int, chart: Chart, prevlist: list[Node]
) -> list[Node]:
    if i + 3 <= j:
        return reduce(
            lambda accl, lnode: reduce(
                lambda accr, rnode: reduce(
                    lambda accc, cnode: CCG.parenthesisRule(lnode, cnode, rnode, accc),
                    lookupChart(i + 1, j - 1, chart),
                    accr.copy(),
                ),
                filter(lambda n: n.cat == cat.RPAREN, lookupChart(j - 1, j, chart)),
                accl.copy(),
            ),
            filter(lambda n: n.cat == cat.LPAREN, lookupChart(i, i + 1, chart)),
            prevlist.copy(),
        )
    else:
...
def checkEmptyCategories(prevlist: list[Node]) -> list[Node]:
    ...
    def update_outer(p: list[Node], ec: Node):
        ...
        result = reduce(lambda l, node: update_inner(l, node, ec), p, p.copy())
        assert result is not None
        return result

    return reduce(update_outer, emptyCategories, prevlist.copy())
...
def extractParseResult(beam: int, chart: Chart) -> ParseResult:
        ...
        match c:
            ...
            case (((i, _), nodes), *_):
                if i == 0:
                    ...
                    return Partial(
                        g(
                            list(map(CCG.wrapNode, sortByNumberOfArgs(nodes))),
                            list(filter(lambda x: x[0][1] <= i, c)),
                        )
                    )
            case _:
                return Failed()
    ...
    def g(
        results: list[Node], cs: list[tuple[tuple[int, int], list[Node]]]
    ) -> list[Node]:
        match cs:
            case []:
                return results
            case (((i, _), nodes), *rest):
                return g(
                    [
                        CCG.conjoinNodes(x, y)
                        for x in map(CCG.wrapNode, nodes)
                        for y in results
                    ][:beam],
                    list(filter(lambda x: x[0][1] <= i, rest)),
                )
    ...
    return f(
        list(
            sorted(
                filter(lambda x: len(x[1]) > 0, chart.items()),
